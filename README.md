...
- **Communication**: named pipe `\\.\pipe\MihaniSecurity` (SDDL restricted to
  System/Administrators/interactive users), newline-delimited JSON messages.
...
- **Token guard** — watches Steam, Discord and browser credential stores;
  verifies the memory of any process that opens them, terminates offenders and
  quarantines their binaries (when the configured severity allows).
...
- **Behavioral detection** — persistence rules (Run keys, Winlogon shell,
  startup folders), DLL/process injection watch, suspicious command lines,
  network beaconing. Each rule maps to a remediation guide in the UI.
...
- **Quarantine** — encrypted store with restore/delete, auto-purge by age and
  size. Log entries are redacted before display (Discord/Steam tokens, Bearer
  headers, passwords, PowerShell `-enc` payloads).
